...
export function setInput(session, input) {
  if (!session) return;
  session.input = String(input || "");
  touch(session);
}

export function setOutput(session, output) {
  if (!session) return;
...
export function addStep(session, title, details) {
  if (!session) return;
...
export function recordExam(session, exam) {
...
export function setExams(session, exams) {
  if (!session) return;
  const list = Array.isArray(exams) ? exams : [];
  session.exams = list.map(exam => ({
    id: exam.id || null,
    label: exam.label || null,
    tipo: exam.tipo || null,
    status: exam.status || null,
...
export function countByStatus(exams) {
  const counts = { alterado: 0, normal: 0, nao_encontrado: 0, outros: 0 };
  (exams || []).forEach(exam => {
    if (!exam || !exam.status) { counts.outros += 1; return; }
  ...
  if (exams.length === 0) {
    ...
    exams.forEach((exam, index) => {
      ...
      if (exam.debug) {
        ...
        lines.push("- referencia utilizada: " + (exam.debug.referenciaUtilizada ? JSON.stringify(exam.debug.referenciaUtilizada) : "N/A"));
